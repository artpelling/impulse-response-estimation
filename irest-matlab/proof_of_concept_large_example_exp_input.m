...
% Generate input -> Let's use half of a Gaussian. There is no rigorous
% proof yet, but the Laguerre coefficients of this monotonically decrease
% (in absolute value) -> A in line 104 should be well conditioned. (BTW proving this should not be too difficult)
t = linspace(0, 5, N);
u = exp(-t.^2/2);
y = causal_conv(h_true, u);

%% Plot poles, true impulse response, input and output
figure;
subplot(221);
plot(real(h_true)); 
grid on;
...
axis square;
subplot(223);
plot(real(u), 'LineWidth', 2); hold on;
plot(imag(u), 'LineWidth', 2); 
legend('real part', 'imaginary part');
title("Input");
grid on;
subplot(224);
plot(real(y), 'LineWidth', 2); hold on;
...
subplot(212);
plot(abs(h - h_true.'), "LineWidth", 2); hold on;
axis tight;
grid on;
xlabel("Discrete timestamps");
...
title("Impulse response comparison");
subplot(212);
plot(abs(h(inds) - h_true(inds).'), "LineWidth", 2); hold on;
axis tight;
grid on;
xlabel("Discrete timestamps");
ylabel("$| h - \hat{h}|$", 'Interpreter', 'latex');
