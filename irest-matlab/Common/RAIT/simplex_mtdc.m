function [p,c] = simplex_mtdc(f, mult, period, inits, show, epsilon)

% SIMPLEX_MTDC - Gives the poles of the discrete complex MT system that 
%                best fit the approximation of the function 'f'.
%
% Usage: 
%     [p,c] = simplex_mtdc(f,mult,period,inits,show,epsilon)
%
% Input parameters:
...
%
% Output parameters:
%     p : predicted poles of the discrete complex MT system with 'mult'
%         multiplicities and 'period' periodicity
%     c : the Fourier coefficients of 'f' with respect to the discrete 
%         complex MT system defined by the predicted poles 'p' 
%
% Copyright: (C) ELTE IK NumAnal, GPL 1.1 ??


% initial simplex
polenum = length(mult); % number of poles
...
for j = 1:ne
    [~,simplex(j,ne)] = mtdc_coeffs(f,periodize_poles(multiply_poles(coords2params(simplex(j,1:n)),mult),period)); % függvényérték az eltérés
end
...
while std(simplex(:,ne)) > epsilon
	...
	[~,fvxr] = mtdc_coeffs(f,periodize_poles(multiply_poles(coords2params(xr),mult),period)); % fvxr
	
	if fvxr < simplex(1,ne) % best function value    
        % examining the extended point
        xe = gamma * xr + (1 - gamma) * xs;
        [~,fvxe] = mtdc_coeffs(f,periodize_poles(multiply_poles(coords2params(xe),mult),period)); % fvxe
	...
	if fvxr > simplex(n,ne)
        ...
        [~,fvxc] = mtdc_coeffs(f,periodize_poles(multiply_poles(coords2params(xc),mult),period)); % fvxc
        ...
        if fvxc < fvxr && fvxc < simplex(ne,ne)
            ...
            for k = 2:ne
                ...
                [~,simplex(k,ne)] = mtdc_coeffs(f,periodize_poles(multiply_poles(coords2params(simplex(k,1:n)),mult),period));
            end
        end
	end
    ...
    if show
        ...
        c = mtdc_coeffs(f,periodize_poles(multiply_poles(coords2params(simplex(1,1:n)),mult),period));
        fs = mtdc_generate(sample,periodize_poles(multiply_poles(coords2params(simplex(1,1:n)),mult),period),c);
        m=sum(mult)*period;
...
c = mtdc_coeffs(f,periodize_poles(multiply_poles(p,mult),period));
