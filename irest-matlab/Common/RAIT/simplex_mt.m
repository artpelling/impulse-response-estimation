function [p,c] = simplex_mt(f, mult, period, inits, show, epsilon)

% SIMPLEX_MT - Gives the poles of the continuous MT system that best fit
%              the approximation of the function 'f'.
%
% Usage: 
%     [p,c] = simplex_mt(f,mult,period,inits,show,epsilon)
%
% Input parameters:
...
%
% Output parameters:
%     p : predicted poles of the continuous MT system with 'mult'
...
for j = 1:ne
    [~,simplex(j,ne)] = mt_coeffs(f,periodize_poles(multiply_poles(coords2params(simplex(j,1:n)),mult),period)); % függvényérték az eltérés
end
...
while std(simplex(:,ne)) > epsilon
	...
	[~,fvxr] = mt_coeffs(f,periodize_poles(multiply_poles(coords2params(xr),mult),period)); % fvxr
	
	if fvxr < simplex(1,ne) % best function value    
        % examining the extended point
        xe = gamma * xr + (1 - gamma) * xs;
        [~,fvxe] = mt_coeffs(f,periodize_poles(multiply_poles(coords2params(xe),mult),period)); % fvxe
	...
	if fvxr > simplex(n,ne)
        ...
        [~,fvxc] = mt_coeffs(f,periodize_poles(multiply_poles(coords2params(xc),mult),period)); % fvxc
        ...
        if fvxc < fvxr && fvxc < simplex(ne,ne)
            ...
            for k = 2:ne
                ...
                [~,simplex(k,ne)] = mt_coeffs(f,periodize_poles(multiply_poles(coords2params(simplex(k,1:n)),mult),period));
            end
        end
	end
    ...
    if show
        ...
        c = mt_coeffs(f,periodize_poles(multiply_poles(coords2params(simplex(1,1:n)),mult),period));
        fs = mt_generate(sample,periodize_poles(multiply_poles(coords2params(simplex(1,1:n)),mult),period),c);
...
c = mt_coeffs(f,periodize_poles(multiply_poles(p,mult),period));
